...
def return_general_summary(results: list[dict], tag: str = "Summary") -> list[dict]:
    '''
    - contextPath: MLPredictPhishingURL.url
    description: URL on which the model ran.
    type: String
    - contextPath: MLPredictPhishingURL.final_verdict
    description: Final verdict of the URL.
    type: String
    - contextPath: MLPredictPhishingURL.is_suspicious_logo
    ...
    type: Boolean
    - contextPath: MLPredictPhishingURL.has_login_form
    ...
    df_summary[KEY_FINAL_VERDICT] = [x.get('verdict') for x in results]  # type: ignore
    summary_context = [
        {'url': x.get('url_redirect'), KEY_CONTENT_SUMMARY_FINAL_VERDICT: BENIGN_VERDICT,
    ...
    return_entry = {
        ...
        "EntryContext": {'MLPredictPhishingURL': summary_context}
    }
    if tag is not None:
...
def get_urls_to_run(
    urls: Union[list[str], str],  model: Model
) -> tuple[list[str], list[str]]:
    urls = [urls] if isinstance(urls, str) else urls
    domain_to_urls = {
        extract_domainv2(url): url
        for url in urls
    }
    urls = list(domain_to_urls.values())
    domains = list(domain_to_urls)
...
def prepare_urls(urls: Union[list[str], str],  model: Model) -> tuple[list[str], list[str]]:
    urls = [urls] if isinstance(urls, str) else urls
    domain_to_urls = {
        domain: url
        for url in urls
...
def main():
    try:
        return_results(get_predictions_for_phishing_urls(**demisto.args()))
    except Exception as e:
        return_error(f'Failed to execute MLPredictPhishingURL. Error: {e}')
